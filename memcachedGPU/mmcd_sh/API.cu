...
__device__ bool TXValidate(TMmetadata *metadata, int timestamp, TXRecord* TxRecords, readSet *rs, int hp, Statistics* stats, int tid, time_rate* times)
{
	...
	for(int lane=0; lane<warpSize; lane++)
	{
        ...
        //if(mylane==0) printf("W%d\t validates lane %d: ts=%d hp=%d anchor=%d tp=%d\n", wid, lane, ts, hp, anchor, metadata->tp);
        //printf("\tT%d nbIters=%d\n", threadIdx.x, nbIters);
...
/*    ///OLD
		...
		for(int i = decrease_pointer_nb(hp,mylane); conflict[wid]==false && i > ; i=decrease_pointer_nb(i,warpSize))
		{
			//if(mylane==0) printf("W%d validates lane %d: range %d-%d, tp=%d hp=%d\n", wid, lane, i, decrease_pointer_nb(i,warpSize), metadata->tp, hp);
			...
			else
			{
                ...
                if (TxRecors[i].ts > anchor) { // TODO :  > is not safe with wrap-aroud
                    conflict=true;
                    break;
                }
...
__device__ int TXAddToRecord(TMmetadata* metadata, TXRecord* TxRecords, readSet* read_log, writeSet* write_log, Statistics* stats, time_rate* times, int timestamp, int tid)
{
	...
	do
	{
		...
		if(get_lane_id() == warp_leader)
		{
			...
			{
				//printf("T%d won the CAS.\twhp=%d->%d tp=%d pv=%d\n", tid, curr_w_hp, metadata->w_hp, metadata->tp, passed_val);
				if((!metadata->hasWrapped) && (curr_w_hp > metadata->w_hp)){
					metadata->hasWrapped=true;
					//printf("\tT%d: wrapped for the FIRST time!\n", tid);
				}
	...
	if(live_tx)
	{
		...
		//printf("\t\tT%d wrote ts %d in position %d of the array\n", tid, newtimestamp, insert);
...
cudaError_t TXInit(uint threadNum, readSet** d_rs, writeSet** d_ws, warpResult** d_wRes)
{
	writeSet*			h_ws;
	readSet*			h_rs;
	warpResult*			h_wRes;
