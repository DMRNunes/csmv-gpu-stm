...
__device__ bool TXValidate(TMmetadata *metadata, int timestamp, uint* txNumber, TXRecord* TxRecords, readSet *rs, int hp, Statistics* stats, int tid, time_rate* times)
{
...
__device__ int TXAddToRecord(TMmetadata* metadata, uint* txNumber, TXRecord* TxRecords, readSet* read_log, writeSet* write_log, Statistics* stats, time_rate* times, int timestamp, int tid)
{
	...
	start_val1 = clock64();
	if(!TXValidate(metadata, timestamp, txNumber, TxRecords, read_log, curr_r_hp, stats, tid, times))
	{
		live_tx=0;
		timestamp=-1;
	}
	else
		timestamp=txNumber[curr_r_hp];
	stop_val1 = clock64();
