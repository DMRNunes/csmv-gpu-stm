...
int main(int argc, char **argv)
{
	...
	*memcd_host.curr_clock = 1;
	...
	TXInit(nbThrs*nbBlks, &d_rs, &d_ws, &d_wRes);
	...
	server_kernel<<<1, nbRecvThrs, 0, s2>>>(gbc_pack, d_rs, d_ws, d_wRes, d_stats, d_times);

	// Populates the cache
	cudaStream_t s1;
	cudaStreamCreateWithFlags(&s1, cudaStreamNonBlocking);
	//memcdWriteTx<<<nbBlks, nbThrs, 0, s1>>>(gbc_pack.gbc[CHANNEL_OFFLOAD], memcd_dev, d_rs, d_ws, d_wRes, d_stats, d_times);
	...
	while(totT_ms < totalDuration*1000)
	{
		...
		memcdTx<<<nbBlks, nbThrs, 0, s1>>>(gbc_pack.gbc[CHANNEL_OFFLOAD], memcd_dev, d_rs, d_ws, d_wRes, seed, prRead, d_stats, d_times);
