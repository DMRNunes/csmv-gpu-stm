...
#define SERV_ARG_DEF TMmetadata* metadata, uint* txNumber, TXRecord* records, readSet* rs, writeSet* ws, warpResult* wRes, Statistics* stats, time_rate* times
#define SERV_ARG metadata, txNumber, records, rs, ws, wRes, stats, times
...
__device__ __forceinline__ void critcal_section(SERV_ARG_DEF, uint val0, int val1)
{
	...
	result=TXAddToRecord(metadata, txNumber, records, rs, ws, stats, times, timestamp, tid);
	...
	for (int i = 0; i < nbTXsPerThread; ++i)
	{
		...
		else
		{
			...
			do
			{
				...
				isAborted=false;
				for (int j = 0; j < nbWays && isAborted==false; ++j) {
					granule_t key =   TXReadOnly(&keys[j + target_set*nbWays], timestamp, rs, ws, tid, &isAborted);
					granule_t state = TXReadOnly(&states[j + target_set*nbWays], timestamp, rs, ws, tid, &isAborted);
					if (key == input_key && (state&VALID)) { // found the key
						granule_t val = TXReadOnly(&values[j + target_set*nbWays], timestamp, rs, ws, tid, &isAborted); // TODO: use the whole val
...
__global__ void server_kernel(gbc_pack_t gbc_pack, TXRecord* records, readSet* rs, writeSet* ws, warpResult* wRes, Statistics* stats, time_rate* times)
{
	...
	while(1)
	{
		worker_thread(gbc_pack, &metadata, txNumber, records, rs, ws, wRes, stats, times);
	}
}
...
int main(int argc, char **argv)
{
	...
	*memcd_host.curr_clock = 1;
	...
	double avg_total=0, avg_runtime=0, avg_commit=0, avg_wb=0, avg_val1=0, avg_val2=0, avg_rwb=0, avg_wait=0, avg_comp=0, avg_pv=0;
