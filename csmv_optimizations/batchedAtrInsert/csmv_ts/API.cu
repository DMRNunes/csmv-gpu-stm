...
__device__ bool TXValidate(TMmetadata *metadata, int timestamp, uint* txNumber, TXRecord* TxRecords, readSet read_log, writeSet write_log, int hp, int oldhp, Statistics* stats, int tid, time_rate* times)
{
	long int comparisons=0;
	

	if(txNumber[metadata->tp] > timestamp)
	{
	...
	for (int i = hp; i != oldhp; i=decrease_pointer(i))
	{
		...
		for(int j=0; j < TxRecords[i].n_writes; j++)
		{
			...
			{
				//if(tid==0) printf("read: %d write: %d\n", read_log.addrs[k], TxRecords[i].writeSet[j]);
...
__device__ int TXAddToRecord(TMmetadata* metadata, uint* txNumber, TXRecord* TxRecords, readSet* read_log, writeSet* write_log, Statistics* stats, time_rate* times, int timestamp, int tid)
{
	...
	start_val1 = clock64();
	if(		(timestamp==-1) || 
			(!TXValidate(metadata, timestamp, txNumber, TxRecords, read_log[tid], write_log[tid], curr_r_hp, metadata->tp, stats, tid, times))
	  )
	{
		live_tx = 0;
	}
	...
	do
	{
		...
		{
			start_val2 = clock64();
			if((live_tx==0) || !TXValidate(metadata, timestamp, txNumber, TxRecords, read_log[tid], write_log[tid], curr_r_hp, old_r_hp, stats, tid, times))
				live_tx=0;
			stop_val2 = clock64();
	...
	if(live_tx)
	{
		...
		newtimestamp=txNumber[decrease_pointer(insert)]+offset+1;
		insert = advance_pointer_nb(insert, offset);

		txNumber[insert] = newtimestamp;
