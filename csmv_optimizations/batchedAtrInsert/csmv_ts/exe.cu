...
__global__ void client_kernel(gbc_t gbc, int *flag, uint64_t seed, uint threadNum, uint dataSize, VertionedDataItem* data, readSet* rs, writeSet* ws, warpResult* wRes, float prRead, int roSize, int upSize, 
	...
	while((*flag & 1)==0)
	{
		...
		do
		{
			...
			#endif
					if(retry==1)value1+=TXReadOnly(data, addr1, timestamp, rs, ws, tid, &isAborted);
				}
				if(isAborted==true)
				{
			...
			else
			{
				for(int i=0; i<upSize && isAborted==false; i++)
				{
					...
					if(retry==1)TXWrite(data, value1-(tid*10+100), addr1, ws, tid);
					...
					if(retry==1)TXWrite(data, value2+(tid*10+100), addr2, ws, tid);
				}
				if(isAborted==true)
				{
					atomicAdd(&(stats->nbAbortsDataAge), 1);
...
__global__ void server_kernel(gbc_pack_t gbc_pack, readSet* rs, writeSet* ws, TXRecord* records, warpResult* wRes, Statistics* stats, time_rate* times)
{
	...
	while(1)
	{
		worker_thread(gbc_pack, &metadata, txNumber, records, rs, ws, wRes, stats, times, stage_buf);
...
///////////////
	...
	if(verbose)
		...
		printf("%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%d\t%d\n", 
			(float)nbAborts/(nbAborts+h_stats->nbCommits)*100.0,
			h_stats->nbCommits/totT_ms*1000.0,
			(float)h_stats->nbAbortsDataAge/(nbAborts+h_stats->nbCommits)*100.0,
			(float)h_stats->nbAbortsRecordAge/(nbAborts+h_stats->nbCommits)*100.0,
