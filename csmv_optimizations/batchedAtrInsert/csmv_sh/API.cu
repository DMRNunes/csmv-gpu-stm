...
__device__ bool TXValidate(TMmetadata *metadata, int timestamp, TXRecord* TxRecords, readSet read_log, writeSet write_log, int hp, int oldhp, Statistics* stats, int tid, time_rate* times)
{
...
__device__ int TXAddToRecord(TMmetadata* metadata, TXRecord* TxRecords, readSet* read_log, writeSet* write_log, Statistics* stats, time_rate* times, int timestamp, int tid)
{
	...
	start_val1 = clock64();
	if(		(timestamp==-1) || 
			(!TXValidate(metadata, timestamp, TxRecords, read_log[tid], write_log[tid], curr_r_hp, metadata->tp, stats, tid, times))
	  )
	{
		live_tx = 0;
	}
	...
	do
	{
		...
		{
			start_val2 = clock64();
			if((live_tx==0) || !TXValidate(metadata, timestamp, TxRecords, read_log[tid], write_log[tid], curr_r_hp, old_r_hp, stats, tid, times))
				live_tx=0;
			else
				timestamp = TxRecords[curr_r_hp].transactionNumber;
			stop_val2 = clock64();
		...
		if(get_lane_id() == warp_leader)
		{
			if(atomicCAS(&(metadata->w_hp), curr_w_hp, advance_pointer_nb(curr_w_hp, passed_val)) == curr_w_hp)
			{
				...
				//printf("W%d had to repeat validation %d times before commiting\n", wid, validations);
