...
#define SERV_ARG_DEF TMmetadata* metadata, TXRecord* records, VertionedDataItem* data, readSet* rs, writeSet* ws, warpResult* wRes, Statistics* stats, time_rate* times
#define SERV_ARG metadata, records, data, rs, ws, wRes, stats, times
...
//printf("%d in server\n", tid);
	...
	result=TXAddToRecord(metadata, records, data, rs, ws, stats, times, timestamp, tid);
...
__global__ void client_kernel(gbc_t gbc, int *flag, uint64_t seed, uint threadNum, uint dataSize, VertionedDataItem* data, readSet* rs, writeSet* ws, warpResult* wRes, float prRead, int roSize, int upSize, 
	...
	while((*flag & 1)==0)
	{
		...
		do
		{
			...
			#endif
					value1+=TXReadOnly(data, i, timestamp, rs, ws, tid, &isAborted);
				}
				if(isAborted==true)
				{
			...
			{
/*				for(int i=0; i<max(upSize,roSize) && isAborted==false; i++)
				{
			#if DISJOINT					
					addr1 = RAND_R_FNC(state)%(max-min+1) + min;
			#else
...
*/
				for(int i=0; i<upSize && isAborted==false; i++)
				{
			...
			#endif
					value1 = TXRead(data, addr1, timestamp, rs, ws, tid, &isAborted); 
					if(retry==1)TXWrite(data, value1-(tid+1), addr1, ws, tid);
					...
					if(retry==1)TXWrite(data, value2+(tid+1), addr2, ws, tid);
				}
				if(isAborted==true)
				{
					atomicAdd(&(stats->nbAbortsDataAge), 1);
...
__global__ void server_kernel(gbc_pack_t gbc_pack, VertionedDataItem* data, readSet* rs, writeSet* ws, warpResult* wRes, Statistics* stats, time_rate* times)
{
	...
	while(1)
	{
		worker_thread(gbc_pack, &metadata, records, data, rs, ws, wRes, stats, times, stage_buf);
...
__global__ void parent_kernel(int *flag, uint total_sender_bk, uint sender_block_size,	uint total_recevier_bk, uint recv_block_size, gbc_pack_t gbc_pack,
	...
	server_kernel<<<total_recevier_bk, recv_block_size, 0, s2>>>(gbc_pack, data, rs, ws, wRes, stats, times);
