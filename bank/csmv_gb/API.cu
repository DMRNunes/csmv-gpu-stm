...
__device__ bool TXValidate(int timestamp, TXRecord* TxRecords, readSet read_log, writeSet write_log, int hp, int tp, int oldhp, Statistics* stats, int tid, time_rate* times)
{
...
__device__ int TXAddToRecord(TMmetadata* metadata, TXRecord* TxRecords, readSet* read_log, writeSet* write_log, Statistics* stats, time_rate* times, int timestamp, int tid)
{
	...
	if(!TXValidate(timestamp, TxRecords, read_log[tid], write_log[tid], curr_r_hp, metadata->tp, metadata->tp, stats, tid, times))
		return 0;
...
//	printf("t%d: validation successful\n", id);
	...
	do
	{
		...
		{
			if(TXValidate(timestamp, TxRecords, read_log[tid], write_log[tid], curr_r_hp, metadata->tp, old_r_hp, stats, tid, times)==false)
			{	
				return 0;
			}
			old_r_hp = curr_r_hp;
...
{	
  	...
  	for(int i=0; i<dataSize; i++)
  	{
  		...
  		*/
  		total += host_data[i].value[host_data[i].head_ptr];
  	}
