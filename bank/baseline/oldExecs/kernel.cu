...
__global__ void bank_kernel(unsigned int seed, int* roArr, unsigned int roSize, unsigned int txSize, unsigned int dataSize, unsigned int threadNum, VertionedDataItem* data, TXRecord* record, TMmetadata* metadata, Statistics* stats, time_rate* times)
{
...
{
	unsigned int blockNum, threads_per_block, roNum, roSize, threadSize, dataSize, seed;
	int *h_ro, *d_ro;
...
//	  "  4) server config - nb threads     \n"
	...
	roNum 				= (atoi(argv[argCnt++])/100.0) * blockNum*threads_per_block;
	...
	cudaEventRecord(start); 		//clock_gettime(CLOCK_REALTIME, &t1);
	bank_kernel<<<gridDist, blockDist>>>(seed, d_ro, roSize, threadSize, dataSize, blockNum*threads_per_block, d_data, records, metadata, d_stats, d_times);
	...
	totT_ms += tKernel_ms;		//clock_gettime(CLOCK_REALTIME, &t2);
	...
	long int denom = (long)threads_per_block*blockNum*peak_clk;
	avg_runtime	/= denom;
	avg_commit 	/= denom;
	avg_wb 		/= denom;
	avg_val 	/= denom;
	avg_rwb 	/= denom;
	avg_comp	/= threads_per_block*blockNum;
