...
__global__ void client_kernel(gbc_t gbc, int *flag, uint64_t seed, uint dataSize, VertionedDataItem* data, readSet* rs, writeSet* ws, warpResult* wRes, float prRead, int roSize, int upSize, 
	...
	int value1, value2, timestamp, addr, result;
	bool isAborted;
		...
		do
		{
			...
			if(rnd < probRead)
			{
				...
				for(int i=0; i<roSize && isAborted==false; i++)
				{
					...
					value1+=TXReadOnly(data, addr, timestamp, rs, ws, tid, &isAborted);
				}
				if(isAborted==true)
				{
			...
			else
			{
				for(int i=0; i<upSize && isAborted==false; i++)
				{
					...
					addr = RAND_R_FNC(state)%dataSize;
					value1 = TXRead(data, addr, timestamp, rs, ws, tid, &isAborted); 
					TXWrite(data, value1-(1), addr, ws, tid);

					addr = RAND_R_FNC(state)%dataSize;
					value2 = TXRead(data, addr, timestamp, rs, ws, tid, &isAborted); 
					TXWrite(data, value2+(1), addr, ws, tid);
				}
				if(isAborted==true)
				{
					atomicAdd(&(stats->nbAbortsDataAge), 1);
